<template>
  <div>
    ...
    <el-form :inline="true" :model="searchObj" class="demo-form-inline" style="margin: 20px 0px 0px 10px">
      ...
      <el-form-item label="用户认证状态">
        <el-select v-model="searchObj.authStatus" placeholder="请选择">
          <el-option
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button v-if="scope.row.status == 1" @click="lock(scope.row.id, 0)" type="danger" size="mini">锁定</el-button>
          <el-button v-if="scope.row.status == 0" @click="lock(scope.row.id, 1)" type="primary" size="mini">解锁</el-button>
        </template>
      </el-table-column>
    </el-table>
