  ...
  <div>
    ...
    <div id="theEvaluationDown" class="theEvaluationDown">
      ...
      <div v-if="orderItem.isReview == 0" id="evaluateDiv${orderItem.product.id}" class="evaluateDiv">
        ...
        <button class="submitBtn">提交评价</button>
      </div>

      <!-- 显示商品评价 -->
      <div v-else>
        <div class="reviewsItem">
          <span class="reviewsDate">${review.strCreateDate}</span>
          <span class="context">${review.content}</span>
          <span class="niMing pull-right">（匿名）</span>
          <span class="userName pull-right">${review.user.anonymousName}</span>
        </div>
      </div>
